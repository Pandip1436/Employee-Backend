...
export class DailyUpdateService {
  ...
  static async getMyUpdates(userId: string, query: { page?: number; limit?: number; month?: string }) {
    ...
    const [data, total] = await Promise.all([
      DailyUpdate.find(filter).populate("userId", "name email department").sort("-date").skip(skip).limit(limit),
  ...
  static async getById(id: string) {
    const update = await DailyUpdate.findById(id).populate("userId", "name email department");
    if (!update) throw new ApiError(404, "Update not found.");
    return update;
  }
}
