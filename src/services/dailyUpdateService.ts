...
export class DailyUpdateService {
  static async create(userId: string, data: {
    ...
    return DailyUpdate.findById(update._id).populate("userId", "name email department role");
  }
  ...
  static async update(id: string, userId: string, data: {
    ...
    await update.save();
    return DailyUpdate.findById(id).populate("userId", "name email department role");
  }
  ...
  static async getMyUpdates(userId: string, query: { page?: number; limit?: number; month?: string }) {
    ...
    const [data, total] = await Promise.all([
      DailyUpdate.find(filter).populate("userId", "name email department role").populate("reviewedBy", "name email").sort("-date").skip(skip).limit(limit),
  ...
  static async getTeamUpdates(query: { page?: number; limit?: number; date?: string; userId?: string }) {
    ...
    const [data, total] = await Promise.all([
      // `role` lets the team view scope updates to employees vs interns.
      DailyUpdate.find(filter).populate("userId", "name email department role").populate("reviewedBy", "name email").sort("-date").skip(skip).limit(limit),
      DailyUpdate.countDocuments(filter),
    ]);
